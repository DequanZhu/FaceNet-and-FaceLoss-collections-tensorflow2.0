...
def create_datasets_from_tfrecord(tfrcd_dir,batch_size=32):
    file_paths = os.listdir(tfrcd_dir)
    file_paths = [os.path.join(tfrcd_dir, file_path)for file_path in file_paths]
    nrof_tfrcd=int(len(file_paths))
    nb_train=nrof_tfrcd*50000
    ...
    train_datasets = parsed_image_dataset.batch(batch_size).prefetch(AUTOTUNE)
    return train_datasets,nb_train
